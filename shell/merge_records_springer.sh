...
#prod
SOURCE_DIRECTORY=/media/lionel/Data/swissbib-data/springer/extracted/
TARGET_DIRECTORY=/home/lionel/Documents/data-swissbib-no-backup/springer/3rd_attempt/
...
{   # merge all the xml files of a subdirectory like 1213/. 
	while read directory; do
		...
		filename="$TARGET_DIRECTORY"springer-"$directory".xml	
		find . -name *.Meta -exec cat {} \; > $filename
		cd ..
		...
		sed -e 's/.*A++V2.4.dtd">//g' -i $filename
